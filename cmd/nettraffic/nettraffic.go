...
func init() {
	...
	NetTrafficCmd.Flags().StringVarP(&app, "limit-to-app", "a", "", "app name to limit Explorer results to flows with that app as a provider or a consumer. default is all apps.")
	...
	NetTrafficCmd.Flags().StringVarP(&exclServiceCSV, "exclude-service-csv", "x", "", "file location of csv with port/protocols to exclude. CSV should have NO HEADERS with port number in column 1 and IANA numeric protocol in Col 2.")
	NetTrafficCmd.Flags().StringVarP(&start, "start", "s", time.Date(time.Now().Year()-5, time.Now().Month(), time.Now().Day(), 0, 0, 0, 0, time.UTC).Format("2006-01-02"), "start date in the format of yyyy-mm-dd. Date is set as midnight UTC.")
	NetTrafficCmd.Flags().StringVarP(&end, "end", "e", time.Now().Add(time.Hour*24).Format("2006-01-02"), "end date in the format of yyyy-mm-dd. Date is set as midnight UTC.")
	NetTrafficCmd.Flags().BoolVar(&exclAllowed, "excl-allowed", false, "excludes allowed traffic flows.")
	NetTrafficCmd.Flags().BoolVar(&exclPotentiallyBlocked, "excl-potentially-blocked", false, "excludes potentially blocked traffic flows.")
	NetTrafficCmd.Flags().BoolVar(&exclBlocked, "excl-blocked", false, "excludes blocked traffic flows.")
	NetTrafficCmd.Flags().SortFlags = false

}

// NetTrafficCmd summarizes flows
...
func nettraffic() {
	...
	// Log start
	utils.Log(0, "started netttraffic command")
	...
	data := [][]string{[]string{"src_ip", "src_hostname", "src_cidr_block", "src_default_gw", "src_role", "src_app", "src_env", "src_loc", "dst_ip", "dst_hostname", "dst_cidr_block", "dst_default_gw", "dst_role", "dst_app", "dst_env", "dst_loc", "policy_status", "date_first", "date_last", "num_flows"}}
	...
	for _, t := range traffic {
		...
		if t.Src.Workload != nil {
			src = []string{t.Src.IP, t.Src.Workload.Hostname, wkldNet(t.Src.Workload.Hostname, t.Src.IP, whm), wkldGW(t.Src.Workload.Hostname, whm), t.Src.Workload.GetRole(pce.LabelMapH).Value,
				t.Src.Workload.GetApp(pce.LabelMapH).Value, t.Src.Workload.GetEnv(pce.LabelMapH).Value, t.Src.Workload.GetLoc(pce.LabelMapH).Value}
		}

		// Destination
		dst := []string{t.Dst.IP, "NA", "NA", "NA", "NA", "NA", "NA", "NA"}
		if t.Dst.Workload != nil {
			dst = []string{t.Dst.IP, t.Dst.Workload.Hostname, wkldNet(t.Dst.Workload.Hostname, t.Dst.IP, whm), wkldGW(t.Dst.Workload.Hostname, whm), t.Dst.Workload.GetRole(pce.LabelMapH).Value,
				t.Dst.Workload.GetApp(pce.LabelMapH).Value, t.Dst.Workload.GetEnv(pce.LabelMapH).Value, t.Dst.Workload.GetLoc(pce.LabelMapH).Value}
		}

		// Append source, destination, policy decision, time stamps, and number of connections to data
		d := append(src, dst...)
	...
	// Write the data
	utils.WriteOutput(data, data, fmt.Sprintf(fmt.Sprintf("workloader-nettraffic-%s.csv", time.Now().Format("20060102_150405"))))

	// Log end
	utils.Log(0, "netttraffic command complete")
...
func wkldNet(hostname, ip string, wkldHostMap map[string]illumioapi.Workload) string {
	if wkld, ok := wkldHostMap[hostname]; ok {
		return wkld.GetCIDR(ip)
	}
	return "NA"
}
